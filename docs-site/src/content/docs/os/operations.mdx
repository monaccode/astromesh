...
<Aside type="tip">
The canonical artifact coordinates and workflow names live in the [`monaccode/astromesh-os`](https://github.com/monaccode/astromesh-os) repository, in its `phase1-publish` workflow. The image is published to **Docker Hub**, not to GHCR.
</Aside>
...
oras pull docker.io/fulfarodev/astromesh-os:0.11.0 -o ./out
ls ./out          # → astromesh-os-phase0.raw.zst (~187 MB compressed)

# `latest` also tracks the newest published version
oras pull docker.io/fulfarodev/astromesh-os:latest -o ./out
```
...
The exact `astromesh` runtime in the image is pinned by ref in `runtime.pin` (currently `v0.35.0`). Bumping it is a deliberate edit, then a rebuild:

```ini
# runtime.pin — prefer a release tag over a floating branch tip
ASTROMESH_REF=v0.35.0
```
