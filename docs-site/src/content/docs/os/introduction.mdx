...
It is built with [mkosi](https://github.com/systemd/mkosi) on **Debian trixie** (kernel 6.12+ LTS). The Astromesh agent runtime (`astromesh`) is baked directly into the image at a pinned commit (currently `v0.35.0`), so a given image is reproducible from one exact ref. The core image lands at **~285 MB**, well under its 500 MB build ceiling.
