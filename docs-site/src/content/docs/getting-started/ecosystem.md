...
|-----------|-------------|----------------|---------|
| **Core Runtime** | Multi-model agent engine with 7 orchestration patterns, per-role model routing, declarative agent chaining, a catalog of 16 declarative integrations, memory, tools, guardrails, and a per-model price table behind cost tracking | `astromesh` | v0.56.2 |
