...
<Aside type="caution" title="Currently in v0.2.0">
...
flowchart TB
    cfg["orbit.yaml"]
    validate["OrbitProvider.validate()"]
    generate["OrbitProvider.generate()  (.tf files from Jinja2 templates)"]
    tfinit["terraform init"]
    tfplan["terraform plan"]
    tfapply["terraform apply"]
    post["Post-provisioning (orbit.env with connection strings)"]
    cfg --> validate --> generate --> tfinit --> tfplan --> tfapply --> post
```
...
|---|---|---|
| **GCP** | v0.2.0 | Cloud Run, Cloud SQL, Memorystore, Secret Manager |
