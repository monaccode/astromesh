...
Estimated monthly cost: ~$15 (Starter preset)
```
...
⚠ This will permanently destroy 10 resources.
...
Written to ./my-terraform/:
  ...
  cloud_run.tf      — Cloud Run service (runtime)
...
Read the runtime service's Cloud Run logs from Cloud Logging.

```
astromeshctl orbit logs [--limit <n>] [--since <duration>]
```
...
| `--since` | string | `1h` | Time window to query, e.g. `30m`, `1h`, `24h`. |
...
```
2026-07-14T10:02:11Z  INFO  Agent run started (agent=support, session=abc123)
2026-07-14T10:02:12Z  INFO  Tool call: web_search
2026-07-14T10:02:14Z  INFO  Agent run completed (200 OK, 1.8s)
```

**Side effects:**
- Read-only — queries Cloud Logging, does not modify any cloud resources

---

## orbit upgrade

Re-render the Terraform templates after an `astromesh-orbit` package update and show a diff against the currently generated files.

```
astromeshctl orbit upgrade [--apply]
```
...
| Flag | Type | Default | Description |
|---|---|---|---|
| `--apply` | bool | false | Write the re-rendered templates to `.orbit/generated/`. Without this flag, only a diff is shown. |
...
```
Re-rendering Terraform templates from installed astromesh-orbit v0.4.0...

--- .orbit/generated/monitoring.tf
+++ .orbit/generated/monitoring.tf (new)
+ resource "google_monitoring_dashboard" "runtime" {
+   ...
+ }

1 file added, 0 changed, 0 removed.
Run with --apply to write these changes.
```

**Side effects:**
- Without `--apply`: read-only, prints a diff
- With `--apply`: overwrites `.orbit/generated/*.tf` (does not run `terraform apply`)
