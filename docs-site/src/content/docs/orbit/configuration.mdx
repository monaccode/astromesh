...
Orbit does not provision a separate vector database for RAG — pgvector runs on the Cloud SQL instance Orbit already deploys. See [GCP Provider](/astromesh/orbit/gcp-provider/) for details.
...
There is no `logging` field — Cloud Run ships container logs to Cloud Logging automatically. See [GCP Provider](/astromesh/orbit/gcp-provider/) for how `orbit logs` reads them.
...
`spec.storage` (v0.3.0) and `spec.observability` (v0.4.0) are implemented today — see [`spec.storage`](#specstorage) and [`spec.observability`](#specobservability) above. The following `spec` sections are still planned for future Orbit versions and are not yet implemented:
