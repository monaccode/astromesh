...
Two additional resources are provisioned by default via `spec.storage` (see [Configuration](/astromesh/orbit/configuration/)):
...
- **Artifact Registry** (`artifact_registry.tf.j2`) — a Docker repository (`<metadata.name>-images` by default) for custom runtime and cloud-api images. Disable with `spec.storage.artifact_registry.enabled: false`.
