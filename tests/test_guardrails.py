...
async def test_pii_redaction_email():
    engine = GuardrailsEngine(config={"input": [{"type": "pii_detection", "action": "redact"}]})
...
async def test_topic_filter_warn():
    engine = GuardrailsEngine(
        config={"input": [{"type": "topic_filter", "blocked_topics": ["competitor"]}]}
    )
...
async def test_output_cost_limit():
    engine = GuardrailsEngine(
        config={"output": [{"type": "cost_limit", "max_tokens_per_turn": 10}]}
    )
