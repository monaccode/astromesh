# Astromech Agent Runtime — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build the Astromech Agent Runtime Platform — a multi-model, multi-pattern AI agent runtime with declarative YAML config, pluggable memory, MCP/tool extensibility, RAG pipelines, ML model registry, and full observability.

**Architecture:** Literal implementation of the NEXUS Agent Runtime Spec v1.0, renamed to Astromech. Four-layer architecture: Agent Layer (YAML configs → Lifecycle Manager), Core Services (Model Router, Memory Manager, Tool Registry), Provider/Storage/Extension Layer, and Infrastructure Layer. All modules are decoupled, configurable, and async.
...
name = "astromech"
version = "0.1.0"
description = "Astromech Agent Runtime Platform — multi-model, multi-pattern AI agent runtime"
readme = "README.md"
...
mcp = ["mcp>=1.0.0"]
all = [
    "astromech[redis,postgres,sqlite,chromadb,qdrant,faiss,embeddings,onnx,observability,mcp]",
]
dev = [
    "pytest>=8.0",
...
- Run: `uv run uvicorn astromech.api.main:app --reload`
...
- All references to "nexus" should be "astromech"
- Config apiVersion: astromech/v1
```

**Step 4: Create astromech/__init__.py**
...
Run: `cd D:/monaccode/astromech-platform && uv sync --all-extras --dev`
Expected: Dependencies installed, `.venv` created

**Step 6: Commit**

```bash
git add pyproject.toml .gitignore CLAUDE.md astromech/__init__.py
...
**Files:**
- Create: `astromech/core/__init__.py`
- Create: `astromech/providers/__init__.py`
- Create: `astromech/providers/base.py`
...
```python
# astromech/core/__init__.py
# astromech/providers/__init__.py
# tests/__init__.py
```
...
```bash
git add astromech/core/ astromech/providers/ tests/
...
**Files:**
- Create: `astromech/core/model_router.py`
...
```python
# astromech/core/model_router.py
import time
...
```bash
git add astromech/core/model_router.py tests/test_model_router.py
...
**Files:**
- Create: `astromech/providers/ollama_provider.py`
...
```python
# astromech/providers/ollama_provider.py
import httpx

from astromech.providers.base import CompletionChunk, CompletionResponse
...
```bash
git add astromech/providers/ollama_provider.py tests/test_providers.py
...
**Files:**
- Create: `astromech/providers/openai_compat.py`
...
```python
# astromech/providers/openai_compat.py
import os
...
```bash
git add astromech/providers/openai_compat.py tests/test_providers.py
...
**Files:**
- Create: `astromech/providers/vllm_provider.py`
...
```python
# astromech/providers/vllm_provider.py
import httpx
...
```bash
git add astromech/providers/vllm_provider.py tests/test_providers.py
...
**Files:**
- Create: `astromech/providers/llamacpp_provider.py`
- Create: `astromech/providers/hf_tgi_provider.py`
- Create: `astromech/providers/onnx_provider.py`
- Modify: `tests/test_providers.py`
...
```python
# astromech/providers/llamacpp_provider.py
import httpx
from astromech.providers.base import CompletionChunk, CompletionResponse
...
```python
# astromech/providers/hf_tgi_provider.py
import httpx
from astromech.providers.base import CompletionChunk, CompletionResponse
...
```python
# astromech/providers/onnx_provider.py
...
class ONNXProvider:
    ...
    async def stream(self, messages: list[dict], **kwargs):
        ...
        from astromech.providers.base import CompletionChunk
...
```bash
git add astromech/providers/ tests/test_providers.py
...
**Files:**
- Create: `astromech/api/__init__.py`
- Create: `astromech/api/main.py`
- Create: `astromech/api/routes/__init__.py`
- Create: `astromech/api/routes/agents.py`
...
```python
# astromech/api/routes/agents.py
...
```python
# astromech/api/__init__.py
# astromech/api/routes/__init__.py
```

**Step 4: Run tests. Step 5: Commit.**

```bash
git add astromech/api/ tests/test_api.py
...
**Files:**
- Create: `astromech/core/tools.py`
...
# tests/test_tools.py
import pytest
from astromech.core.tools import ToolDefinition, ToolRegistry, ToolType
...
```bash
git add astromech/core/tools.py tests/test_tools.py
...
**Files:**
- Create: `astromech/core/prompt_engine.py`
...
def test_render_simple_template():
    engine = PromptEngine()
    result = engine.render("Hello {{ name }}!", {"name": "Astromech"})
    assert result == "Hello Astromech!"
...
def test_register_and_render_template():
    ...
    result = engine.render_template("greeting", {"name": "User", "place": "Astromech"})
    assert result == "Hello User, welcome to Astromech!"
...
```python
# astromech/core/prompt_engine.py
...
```bash
git add astromech/core/prompt_engine.py tests/test_prompt_engine.py
...
**Files:**
- Create: `astromech/orchestration/__init__.py`
- Create: `astromech/orchestration/patterns.py`
...
```python
# astromech/orchestration/patterns.py
...
```bash
git add astromech/orchestration/ tests/test_patterns.py
...
**Files:**
- Create: `astromech/core/memory.py`
- Create: `astromech/memory/__init__.py`
...
```bash
git add astromech/core/memory.py astromech/memory/ tests/test_memory.py
...
**Files:**
- Create: `astromech/runtime/__init__.py`
- Create: `astromech/runtime/engine.py`
...
from astromech.runtime.engine import AgentRuntime, Agent
...
```python
# astromech/runtime/engine.py
from pathlib import Path

import yaml

from astromech.core.memory import MemoryManager
from astromech.core.model_router import ModelRouter
from astromech.core.prompt_engine import PromptEngine
...
```bash
git add astromech/runtime/ config/ tests/test_engine.py
...
**Files:**
- Create: `astromech/memory/backends/__init__.py`
- Create: `astromech/memory/backends/redis_conv.py`
...
async def test_redis_conv_save_and_get():
    with patch("astromech.memory.backends.redis_conv.aioredis") as mock_redis_mod:
...
```python
# astromech/memory/backends/redis_conv.py
import json
from datetime import datetime
...
```bash
git add astromech/memory/ tests/test_memory_backends.py
...
**Files:**
- Create: `astromech/memory/backends/sqlite_conv.py`
...
**Files:**
- Create: `astromech/memory/backends/pg_conv.py`
- Modify: `tests/test_memory_backends.py`
...
**Files:**
- Create: `astromech/memory/backends/pgvector_sem.py`
- Modify: `tests/test_memory_backends.py`
...
**Files:**
- Create: `astromech/memory/backends/chroma_sem.py`
...
**Files:**
- Create: `astromech/memory/backends/qdrant_sem.py`
...
**Files:**
- Create: `astromech/memory/backends/faiss_sem.py`
...
**Files:**
- Create: `astromech/memory/backends/pg_episodic.py`
...
**Files:**
- Create: `astromech/memory/strategies/__init__.py`
- Create: `astromech/memory/strategies/sliding_window.py`
- Create: `astromech/memory/strategies/summary.py`
- Create: `astromech/memory/strategies/token_budget.py`
...
**Files:**
- Create: `astromech/rag/__init__.py`
- Create: `astromech/rag/chunking/__init__.py`
- Create: `astromech/rag/chunking/fixed.py`
- Create: `astromech/rag/chunking/recursive.py`
- Create: `astromech/rag/chunking/semantic.py`
- Create: `astromech/rag/chunking/sentence.py`
...
**Files:**
- Create: `astromech/rag/embeddings/__init__.py`
- Create: `astromech/rag/embeddings/hf.py`
- Create: `astromech/rag/embeddings/st.py`
- Create: `astromech/rag/embeddings/ollama.py`
...
**Files:**
- Create: `astromech/rag/stores/__init__.py`
- Create: `astromech/rag/stores/pgvector.py`
- Create: `astromech/rag/stores/chroma.py`
- Create: `astromech/rag/stores/qdrant.py`
- Create: `astromech/rag/stores/faiss_store.py`
...
**Files:**
- Create: `astromech/rag/reranking/__init__.py`
- Create: `astromech/rag/reranking/cross_encoder.py`
- Create: `astromech/rag/reranking/cohere.py`
- Create: `astromech/rag/pipeline.py`
...
**Files:**
- Modify: `astromech/orchestration/patterns.py`
...
**Files:**
- Create: `astromech/orchestration/supervisor.py`
...
**Files:**
- Create: `astromech/orchestration/swarm.py`
...
**Files:**
- Modify: `astromech/runtime/engine.py` — add all patterns to pattern_map
...
**Files:**
- Create: `astromech/mcp/__init__.py`
- Create: `astromech/mcp/client.py`
...
**Files:**
- Create: `astromech/mcp/server.py`

**Commit:** `"feat: add MCP server to expose Astromech agents as tools"`
...
**Files:**
- Modify: `astromech/core/tools.py` — add register_mcp_server
- Modify: `astromech/runtime/engine.py` — load MCP servers from agent config
...
**Files:**
- Create: `astromech/ml/__init__.py`
- Create: `astromech/ml/model_registry.py`
...
**Files:**
- Create: `astromech/ml/serving/__init__.py`
- Create: `astromech/ml/serving/onnx_runtime.py`
- Create: `astromech/ml/serving/torch_serve.py`
...
**Files:**
- Create: `astromech/ml/training/__init__.py`
- Create: `astromech/ml/training/classifier.py`
- Create: `astromech/ml/training/embeddings.py`
...
**Files:**
- Create: `astromech/observability/__init__.py`
- Create: `astromech/observability/telemetry.py`
...
**Files:**
- Create: `astromech/observability/metrics.py`
...
**Files:**
- Create: `astromech/observability/cost_tracker.py`
...
**Files:**
- Create: `astromech/core/guardrails.py`
...
**Files:**
- Modify: `astromech/api/routes/agents.py` — full CRUD
- Create: `astromech/api/routes/memory.py`
- Create: `astromech/api/routes/tools.py`
- Create: `astromech/api/routes/rag.py`
- Modify: `astromech/api/main.py` — register all routers
...
CMD ["uv", "run", "uvicorn", "astromech.api.main:app", "--host", "0.0.0.0", "--port", "8000"]
```

**Step 2: Write docker-compose.yaml**

Full stack with all services from the design doc: astromech, ollama, vllm, embeddings, reranker, postgres, redis, otel-collector, prometheus, grafana.
...
_(These are example configs matching the spec, with `apiVersion: astromech/v1`)_
