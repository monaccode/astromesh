...
- **`praxis_lca`: el «no» a la oferta de membresía se registra.**
  `responder_oferta_membresia` creaba el `lca_pendiente` `oferta_rechazada` ya
  en `resuelto`, y PRAXIS rechaza crear un registro fuera del estado inicial
  (`abierto`): todo «no» fallaba y la regla de no volver a ofrecer por treinta
  días nunca se activaba. Ahora nace `abierto` y se cierra con un `PATCH`; si
  el cierre falla, se informa en vez de tragarse.
